...
const STORAGE_KEY = '@dependente_settings';

const ConfiguracoesDependente = ({navigation}) => {
  const [settings, setSettings] = useState({
    notificacoes: true,
    notificacaoSom: true,
    vibracao: true,
  });
  const [dependenteInfo, setDependenteInfo] = useState(null);
  ...
  useEffect(() => {
    ...
    const loadDependenteInfo = async () => {
      try {
        ...
        console.error('Erro ao carregar informações do dependente:', error);
      }
    };
  ...
  const settingsData = [
    {
      ...
      items: [
        {
          id: 'nome',
          title: dependenteInfo?.nome || dependenteInfo?.nomeCompleto || 'Usuário',
          description: 'Conta de dependente',
          icon: 'person',
          type: 'info',
          color: '#10B981',
        },
        {
          id: 'email',
          title: dependenteInfo?.email || auth().currentUser?.email || 'Email não disponível',
      ...
      items: [
        {
          id: 'logout',
          title: 'Sair da Conta',
          description: 'Encerrar sessão e voltar ao login',
          icon: 'log-out',
          type: 'action',
          color: '#EF4444',
          action: handleLogout,
  ...
  const renderSettingItem = item => {
    ...
    return (
      <TouchableOpacity
        ...
        disabled={isDisabled || item.type === 'info'}
        activeOpacity={item.type === 'info' ? 1 : 0.8}>
...
export default ConfiguracoesDependente;